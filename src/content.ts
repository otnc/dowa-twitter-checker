...
function analyzeTweet(tweet: HTMLElement) {
  ...
  const tweetTextEl = candidates.find((el) => el.closest('article') === tweet) as
  ...
  for (const m of mutations) {
    for (const node of Array.from(m.addedNodes)) {
      ...
      if (node.matches && node.matches('article[data-testid="tweet"]'))
        analyzeTweet(node as HTMLElement);
      const articles =
        node.querySelectorAll && node.querySelectorAll('article[data-testid="tweet"]');
      if (articles) articles.forEach((a) => analyzeTweet(a as HTMLElement));
    }
